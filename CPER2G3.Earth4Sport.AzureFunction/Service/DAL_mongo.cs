...
namespace CPER2G3.Earth4Sport.AzureFunction.Service {
    public class DAL_mongo : IDAL {
        ...
        public async Task<ObjectResult> getSessionsList(string clockUuid) {
            ...
            try {
                ...
                foreach (var session in sessionList) {
                    var data = AllSessionsData.Find(s => s.SessionUUID == session).SortBy(s => s.TimeStamp).ToList();
                    SessionSummary summary = new SessionSummary(data, session);
                    if (summary.TotalDistance != 0) {
                        res.Add(summary);
                    }
                }
                res = res.OrderBy(s => s.Start).ToList();
        ...
        public async Task<ObjectResult> getSessionActivities(string sessionUuid, string clockUuid) {
            ...
            try {
                var sessionCollection = collection.Find(s => s.SessionUUID == sessionUuid).SortBy(s => s.TimeStamp).ToList();
